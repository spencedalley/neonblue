...
class VariantConfig(BaseModel):
    ...
    variant_name: str
    traffic_allocation_percent: float = Field(..., ge=0.0, le=100.0, description="Percentage of traffic allocated to this variant.")
...
class ExperimentModel(BaseModel):
    ...
    status: str = Field(..., description="e.g., 'DRAFT', 'RUNNING', 'PAUSED', 'COMPLETED'")
    start_time: datetime = Field(default_factory=datetime.utcnow)
    end_time: Optional[datetime] = None
    variants: List[VariantConfig]
    total_traffic_allocation: float = Field(..., description="Sum of all variant traffic_allocation_percent, should be 100.0.")
...
class ExperimentVariantConfigResponseModel(BaseModel):
    variant_id: str
    variant_name: str
    traffic_allocation_percent: float = Field(..., ge=0.0, le=100.0,
...
class EventModel(BaseModel):
    ...
    user_id: str
    experiment_id: Optional[str] = Field(None, description="Optional: ID of the experiment the user is currently participating in (at event time).")
    type: str = Field(..., description="e.g., 'click', 'purchase', 'signup'")
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    properties: Dict = Field(default_factory=dict, description="Flexible JSON object for additional context (e.g., 'product_id', 'purchase_amount').")
...
class VariantMetric(BaseModel):
    ...
    metric_name: str = Field(..., description="e.g., 'conversion_rate', 'total_revenue', 'user_count'")
    value: float
...
class VariantResult(BaseModel):
    ...
    variant_name: str
    assignment_count: int = Field(..., description="Total users assigned to this variant *before* the reporting period end.")
    active_user_count: int = Field(..., description="Users who recorded relevant events during the reporting period.")
