...
class ExperimentRepository:
    ...
    def create_experiment(self, experiment_data: ExperimentCreateModel) -> ExperimentORM:
        """
        ...
        total_allocation = sum(v.traffic_allocation_percent for v in experiment_data.variants)
        if total_allocation != 100.0:
            # Raise an appropriate error for the service layer to catch
            raise ValueError(f"Total traffic allocation must be 100%. Got: {total_allocation}%")
        ...
        try:
            ...
            with self.db.begin():
                ...
                experiment_data_dict = experiment_data.model_dump(exclude={'variants'}, exclude_unset=True)
                ...
                for variant_data in experiment_data.variants:
                    ...
                    variant_dict['experiment_id'] = experiment_id  # ⬅️ Link to the parent
                    ...
                    if variant_dict.get('configuration_json') is not None:
                        ...
                        variant_dict['configuration_json'] = json.dumps(variant_dict['configuration_json'])
            ...
            raise RuntimeError(f"A database error occurred during experiment creation: {e}")

        except Exception:
            # The 'with' block handles rollback for any exception
            raise

    def get_experiment_with_variants(
            self,
            experiment_id: str
    ) -> ExperimentORM | None:
        """
        ...
        stmt = stmt.options(
            joinedload(ExperimentORM.variants)
        )
